...
-- Schema sai
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `sai` DEFAULT CHARACTER SET utf8 ;
USE `sai` ;
...
CREATE TABLE IF NOT EXISTS `sai`.`family_callsign` (
  ...
  INDEX `fk_family_callsign_user1_idx` (`to_user_id` ASC) VISIBLE,
  CONSTRAINT `FK_USER_TO_FAMILY_CALLSIGN_1`
...
CREATE TABLE IF NOT EXISTS `sai`.`family_register` (
  ...
  INDEX `fk_family_register_user1_idx` (`approve_user_id` ASC) VISIBLE,
  ...
  CONSTRAINT `FK_USER_TO_FAMILY_REGISTER_1`
    FOREIGN KEY (`user_id`)
    REFERENCES `sai`.`user` (`user_id`),
  CONSTRAINT `fk_family_register_user1`
...
CREATE TABLE IF NOT EXISTS `sai`.`notification` (
  ...
  `noti_from_user_id` VARCHAR(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `sai`.`poll_result` (
  ...
  INDEX `fk_poll_result_user1_idx` (`user_user_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `sai`.`reply` (
  ...
  INDEX `fk_reply_user1_idx` (`user_user_id` ASC) VISIBLE,
